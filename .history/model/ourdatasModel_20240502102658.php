...
function getAllNosdatas(PDO $db, $order = "DESC"): array|string
{
...
function getOneNosdatasByID(PDO $connexion, int $id): array|string
{
...
function deleteNosdatasByID(PDO $db, int $idPomme) : bool|string
{
...
function updateNosdatasByID(
        ...
        : bool|string
{
    $sql = "UPDATE `localisations` SET `nom` = :nom, `adresse` = :adresse,`:codepostal` = :codepostal `latitude` = :latitude, `longitude` = :longitude WHERE `idNosdatas` = :id";
    $prepare = $db->prepare($sql);
    try{
                    ...
                    ) : bool|string
{
    ...
    try{
        $prepare->execute([$nom, $adresse,$codepostal, $latitude, $longitude]);
        $prepare->closeCursor();
        return true;
    }catch(Exception $e){
        return $e->getMessage();
